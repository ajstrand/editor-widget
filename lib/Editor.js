...
var clipboard = Promise.promisifyAll(copyPaste);
...
class Editor {
  constructor(opts) {
      ...
      _.merge(
        {
          ...
          style: {},
          top: 0,
          left: self.options.multiLine && !self.gutter.options.hidden
            ? self.gutter.width
            : 0,
          right: 0,
          bottom: 0,
    ...
    self._updateContent = lodash.throttle(
      _updateContent,
    ...
    self.textBuf.setTextInRange(
      ...
      util.text
        ...
        .map(function (line, i) {
          ...
          if (i === 0)
            startDiff = result.length - line.length;
        ...
        while (paragraphs--) {
          while (true) {
            ...
            if (!(0 <= cursor.row && cursor.row < self.textBuf.getLastRow()))
              break;
            if (/^\s*$/g.test(self.textBuf.lineForRow(cursor.row)))
              break;
          }
        }
      } else {
  ...
  moveCursorHorizontal(count, words) {
    ...
    if (words) {
      ...
      while (words--) {
        ...
        var wordMatch = word[direction === -1 ? "prev" : "current"](
          line,
          cursor.column
        );
  ...
  matchingBracket(pos) {
    ...
    var bracket = (self.lineWithEndingForRow(pos.row)[pos.column] || "").match(
    ...
    var lines = util.text.splitLines(
  ...
  _initHighlighting() {
    ...
    self.data.highlight = { ranges: [], revision: 0, bucket: spawn.getBucket() };

    if (!Editor.count++)
      Editor.highlightClient = spawn().tap(function (client) {
        var loggerOpts = self.options.logger;
        if (loggerOpts)
          client.send({ type: "logger", options: loggerOpts });
      });
    ...
    Editor.highlightClient.done(function (client) {
      ...
      client.once("message", function highlight(data) {
        ...
        if (data.bucket === self.data.highlight.bucket &&
  ...
  _initHandlers() {
    ...
    self.on("keypress", function (ch, key) {
      ...
      if (binding &&
        ["go", "select", "delete"].some(function (action) {
            ...
            return [
              ...
              if (directionDistance.indexOf(direction.name) === 0) {
                ...
                if (moved) {
                  if (action === "go")
                    selection.clearTail();
                  if (action === "delete" && !self.readOnly())
                    self.delete();
                  return true;
                }
              }
        ...
        switch (binding) {
          ...
          case "duplicateLine":
            ...
            if (line === self.textBuf.lineForRow(cursor.row))
              line = "\n" + line;
            var nextLinePos = new Point(cursor.row + 1, 0);
            self.textBuf.setTextInRange(
              new Range(nextLinePos, nextLinePos),
              line
            );
            return false;
          case "undo":
    ...
    self.on("mouse", function (mouseData) {
      ...
      if (mouseData.action === "mouseup")
        self.data.lastClick = { mouse: mouse, time: Date.now() };
      if (mouseData.action === "mousedown") {
        ...
        if (lastClick &&
          ...
          if (current) {
            ...
            } else if (current.index <= mouse.column &&
          ...
          if ((self._lastMouseData || {}).action !== "mousedown" &&
            !mouseData.shift)
    ...
    selection.onDidChange(function () {
      ...
      if (self.data.updatePreferredX)
        self.data.preferredCursorX = cursor.column; // preferred X when moving vertically
      self._markMatches();
...
Editor.prototype._updateContent = function () {
  ...
  util.text
    ...
    .forEach(function (line, row) {
      ...
      var renderableLineEnding = self._renderableLineEnding(
